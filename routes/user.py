...
def add_user():
    ...
    cursor.execute("SELECT usrlst_user_group_id FROM user_list WHERE usrlst_id=%s", (admin_id,))
    ...
    if not all([name, email, contact, department, business_unit, escalation_mail]):
        return jsonify({"error": "All fields are required"}), 400
